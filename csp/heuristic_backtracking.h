...
{
	template <typename T>
	using PrimarySelector = std::function<std::vector<Ref<Variable<T>>>(ConstraintProblem<T>& constraintProblem)>;
	template <typename T>
	using SecondarySelector = std::function<Variable<T>&(ConstraintProblem<T>&, const std::vector<Ref<Variable<T>>>& candidateVars)>;
	template <typename T>
...
namespace csp
{
		...
		AssignmentHistory<T>& assignmentHistory)
	{
		...
		for (T value : sortedDomain)
		{
			...
			if (__heuristicBacktrackingSolver<T>(constraintProblem, primarySelector, secondarySelector, optDomainSorter, optInference,
